...
const OrderRefunds = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      setOrders(tempData.filter(order => order.data.status === 'refund'));
    } catch (error) {
  ...
  const handleReissue = async (orderId) => {
    try {
      // Example action: Mark the order as 'reissued' or handle it as needed
      await firestore().collection('orders').doc(orderId).update({ status: 'reissued' });
      console.log(`Order ${orderId} status updated to 'reissued'`);
      
      // Update local state
      setOrders(prevOrders =>
        prevOrders.map(order =>
          order.orderId === orderId
            ? { ...order, data: { ...order.data, status: 'reissued' } }
            : order
        )
      );
    } catch (error) {
  ...
  const renderOrderItem = ({ item }) => {
    ...
    return (
      <View style={styles.orderItem}>
        ...
        <TouchableOpacity
          style={[styles.reissueButton, isExpanded && styles.expandedReissueButton]}
          onPress={() => handleReissue(item.orderId)}
        >
          <Text style={styles.reissueButtonText}>Reissue</Text>
        </TouchableOpacity>
      </View>
    );
  };
